...
type gctx = {
  ...
  is_thick: bool; (*are the lines thick??*)
  }
...
let with_thickness (g: gctx) (thickness: bool) : gctx = 
  {g with is_thick = thickness}
...
let set_graphics_state (gc: gctx) : unit =
  ...
  (*If the pen_mode is thick, then make the width is 3 else its default 1*)
  Graphics.set_line_width (if gc.is_thick then 3 else 1)
...
(*Draws a specific point at position p within context g*)
let draw_point (g: gctx) (p: position) : unit = 
  set_graphics_state g;
  let (x1, y1) = ocaml_coords g p in 
  Graphics.plot x1 y1

(*Draws a series of points based on the position list within g*)
let draw_points (g: gctx) (p_list: position list): unit = 
  List.iter (draw_point g) p_list
...
(*
(*Sets the line thickness to thick 3*)
let make_thick (g: gctx) = 
  set_graphics_state g;
  Graphics.set_line_width 3
  
(*Sets the line thickness to thin 1*)
let make_thin (g: gctx) = 
  set_graphics_state g;
